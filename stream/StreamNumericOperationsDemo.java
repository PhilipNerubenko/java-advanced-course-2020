...
public class StreamNumericOperationsDemo {
  ...
  public static void main(String[] args) {
    ...
    students.stream()
        ...
        .forEach(element
            -> System.out.println(
                "Student older than 20: " + element.getName() + ", Age: " + element.getAge()));
    ...
    double average =
        students.stream().mapToInt(element -> element.getCourse()).average().getAsDouble();
    System.out.println("Average of courses: " + average);

    int min1 = students.stream().mapToInt(element -> element.getCourse()).min().getAsInt();
    System.out.println("Minimum of courses: " + min1);

    int max1 = students.stream().mapToInt(element -> element.getCourse()).max().getAsInt();
